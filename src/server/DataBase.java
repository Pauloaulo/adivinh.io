...
public class DataBase {
    ...
    public void verifyInput (String category, String expected, String input) {
        input = input.toLowerCase(Locale.ROOT);

        HashMap<String, HashSet<String>> categoryMap = data.get(category);
        if (input.equals(expected)) {
            System.out.printf("%s!!!%n\n", input);
        } else {
            HashSet<String> variables = categoryMap.get(expected);
